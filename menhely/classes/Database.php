...
class Database {
    ...
    public function register($szigszam, $usersnames, $emails, $name, $pass,) {
        //$password = password_hash($pass, PASSWORD_ARGON2I);
        $stmt = $this->db->prepare("INSERT INTO `users` (`szigszam`, `usernames`, `emails`, `name`, `password`) VALUES (?,?,?,?,?);");
        $stmt->bind_param("ss", $szigszam, $usersnames, $emails, $name, $pass,);
        if ($stmt->execute()) {
